...
#menuContainer
    router-link#Lego(to='/')
        img(:src='iconSrc')
    h1.TitleB {{name}}
    .menuItem(v-for='item in menuList' @click='changePath(item.name)') {{item.name}}
        #underline
    #drawer(v-if="drawerStatus")
        .drawerItem(v-for='(item,i) in menuList' @click='opentheDrawer(),changePath(item.name)') 
            a {{item.name}}
                #underline
...
export default {
    ...
    methods: {
        ...
        changePath: function(name){
            this.$router.push({ path: '/section/'+ name})
        },
        ApiListMenus(id) {
            ListMenus(id)
                .then(response => {
